<script lang="ts">
    ...
    const countStats = statUtils.getCountStats(stats);
    const areaStats = statUtils.getAreaStats(stats);
    ...
    const countData = {
        ...
        datasets: [
            {
                label: "Count",
                data: Object.values(countStats) as number[],
                backgroundColor: COLORS_PER_FEATURE.SUBTOTAL,
                hoverBackgroundColor: Object.keys(areaStats).map((label) => COLORS_PER_FEATURE[label.toUpperCase()]),
    ...
    const areaWaterfallData = {
        ...
        datasets: [
            {
                label: "Count",
                data: Object.values(areaStats) as number[],
                backgroundColor: COLORS_PER_FEATURE.SUBTOTAL,
                hoverBackgroundColor: Object.keys(areaStats).map((label) => COLORS_PER_FEATURE[label.toUpperCase()]),
                borderColor: COLORS_PER_FEATURE.SUBTOTAL,
                borderWidth: 1,
...
<div class="stats-charts">
    <div class="chart">
        <Bar data={countData} options={{ indexAxis: "y", ...horizontalChartOptions }} />
    </div>
